...
(*******************************************************************************
  Процедура управление объектами
*******************************************************************************)
...
begin
  ...
  begin
    try
      ...
      xMemory.SaveToFile(ExtractFilePath(ParamStr(0)) + '_' + AName + '.txt');
      xMemory.Position := 0;
      ...
      begin
        ...
        {$ENDIF}
        // -------------------------
        xParam := Params.Items[0];
        xParam.DataType := ftString;
        xParam.Value := ADocID;
        // -------------------------
        xParam := Params.Items[1];
        xParam.DataType := ftString;
        xParam.Value := AName;
        // -------------------------
        xParam := Params.Items[2];
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        xName := xDataSet.FieldByName('name').AsString;
        xField := xDataSet.FieldByName('value');
