...
type
  ...
  TWeightedValueList = TList<TWeightedValue>;

  TTiketsMainFrame = class(TFrame, IModule)
    Text: TText;
    StrGrid: TStringGrid;
    Memo: TMemo;
  private
...
procedure TTiketsMainFrame.SetWeightedValue;
...
constructor TTiketsMainFrame.Create(AOwner: TComponent);
...
destructor TTiketsMainFrame.Destroy;
begin
...
procedure TTiketsMainFrame.AddWeightedCount(const ACount: Integer);
var
  xW: TWeightedValue;
begin
  xW.Count := ACount;
...
function TTiketsMainFrame.GetCaption: WideString;
begin
  Result := 'Торговая система на основе тикетов';
end;

function TTiketsMainFrame.Start: Boolean;
begin
  Result := True;
  FTikets.Clear;
...
function TTiketsMainFrame.Stop: Boolean;
begin
  Result := True;
end;

function TTiketsMainFrame.UpData: Boolean;
var
  xTiket: TTiket;
begin
  Result := True;
...
procedure TTiketsMainFrame.SetGenerationEvent;
var
  xTiket: TTiket;
  xSumStatus: Integer;
  xW: TWeightedValue;
...
procedure TTiketsMainFrame.SetTiket(const ATiket: TTiket);
begin
...
procedure TTiketsMainFrame.SetTikets;
var
  xW: TWeightedValue;
  i, iCount: Integer;
begin
...
initialization
  RegistrationFrame('TIKETS',TTiketsMainFrame);
