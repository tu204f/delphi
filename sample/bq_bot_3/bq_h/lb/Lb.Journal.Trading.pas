...
type
  ...
  private
    procedure CloseOrder(APrice: Double; ATypeTrade: TTypeTrade);
    ...
    case FSide of
      tsBuy: begin
        if APrice < FStopLoss then
          CloseOrder(APrice,TTypeTrade.ttCloseSL);
      end;
      tsSell: begin
        if APrice > FStopLoss then
          CloseOrder(APrice,TTypeTrade.ttCloseSL);
...
begin
  ...
  if Self.Profit < (-1 * FRatesSL) then
    CloseOrder(APrice, TTypeTrade.ttCloseRateSL);
end;
...
begin
  ...
  if Self.Profit > FRatesTK then
    CloseOrder(APrice, TTypeTrade.ttCloseRateTK);
end;
